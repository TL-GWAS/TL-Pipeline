...
process FilterBED {
    ...
    script:
        ...
        qc_file = qcfile instanceof File && qcfile.getName() != 'NO_QC_FILE' ? "--qc-file ${qcfile}" : ''
        ld_blocks = ld_blocks instanceof File && ld_blocks.getName() != 'NO_LD_BLOCKS' ? "--ld-blocks-file ${ld_blocks}" : ''
        """
        TEMPD=\$(mktemp -d)
